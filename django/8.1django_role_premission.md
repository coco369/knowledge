...
查询用户所有的权限：user.get\_all\_permissions()方法列出用户的所有权限，返回值是permission name

查询用户的组权限：user.get\_group\_permissions()方法列出用户所属group的权限，返回值是permission name
...
语法：@permission_required('模型名.权限名codename')
